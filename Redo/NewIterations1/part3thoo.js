...
function InfoObj( id, y, clr, dataSize, time )
{
	...
	//WAVE INFOTYPE
	this.drawWave = function( dataSize, endPoints )
	{
		...
		for( i = 0; i < this._w; i += 3 )
		{
			...
			{
				ellipse( i * ( dataSize/24 ), this._h/4 * sin( freq * ( this._time + i ) ) + this._y - (this._h / 2), dataSize, dataSize );
			}
			else if( i < endPoints[ 3 ] )
			{
				ellipse( i * ( dataSize/6 ), this._h/4 * sin( freq * ( this._time + i ) ) + this._y - (this._h / 2), dataSize, dataSize );
			}
		}
	}
	...
	this.parseData = function( xValue1, xValue2, xValue3 )
	{
		...
		if( id == 0 )
		{
			...
			this.drawWave( dataSize, endPoints );
