...
function InfoObj( id, y, clr, dataSize, time )
{
	...
	//BINARY INFOTYPE
	this.drawBinary = function( dataSize, endPoints, skew1, skew2 )
	{
		for( i = 0; i < this._w; i += 8 )
		{
			...
			if( i < endPoints[ 0 ] )
			{
				if( i % 32 == 0 )
				{
					...
					noFill();
					strokeWeight( 2 );
					ellipse( i * dataSize/10, this._y + random(skew2), dataSize * (1.5*skew1), dataSize * (2*skew1) );
				}
				else if( i % 16 == 0 )
				{
					// "1" part of binary
					...
					fill( this._clr );
					strokeWeight( 2 );
					rect( i * dataSize/10, this._y + random(skew2), dataSize * skew1, dataSize * (2*skew1) );
				}
			}
			else if( i < endPoints[ 1 ] )
			{
	...
	//LETTER INFOTYPE
	this.drawLetters = function( dataSize, endPoints )
	{
	...
	//PICTO INFOTYPE
	this.drawPicto = function( dataSize, endPoints )
	{
		for( i = 0; i < this._w; i += 16 )
		{
			if( i < endPoints[ 0 ] )
	...
	this.parseData = function( xValue1, xValue2, xValue3 )
	{
		...
		else if( id == 1 )
		{
			...
			this.drawBinary( dataSize, endPoints, skew1, skew2 );
		}
		else if( id == 2 )
		{
			// when doing skews, make i < this._w turn to a set i < stopPoint1 which is this._w by default
			textSize( dataSize * 2 );
			this.drawLetters( dataSize, endPoints );
		}
		else if( id == 3 )
		{
			// when doing skews, make i < this._w turn to a set i < stopPoint1 which is this._w by default
			this.drawPicto( dataSize, endPoints );
		}
		this._time += 0.1;
	}
